...
def train():
    train_data = GraphEditDatasetCombinationSeparateLabel(conf, "train") 
    val_data = GraphEditDatasetCombinationSeparateLabel(conf, "val")
    ...
    while run < conf.training.num_epochs and counter <= patience:
        model.train()
        ...
        for i in range(n_batches):
            ...
            optimizer.zero_grad()
            out = model(batch_data, batch_data_sizes, one_hot_labels, query_adj, target_adj)
