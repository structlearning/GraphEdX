...
class ABL_MAX(torch.nn.Module):
    def __init__(self, conf, gmn_config):
        super(ABL_MAX, self).__init__()
        self.conf = conf
    ...
    def forward(self, graphs, graph_sizes, query_adj, corpus_adj):
        ...
        # pairwise_or = alpha_query[:, :, None] | alpha_corpus[:, None, :]
        ...
        if self.use_m_ms_edge:
            ...
            edge_emb_pairwise_max = torch.maximum(nC2_query_edge_embeddings[:, :, None, :], nC2_corpus_edge_embeddings[:, None, :, :])
            edge_align_dist = (0.5 * (self.edge_del_cost + self.edge_ins_cost) * edge_emb_pairwise_max * edge_transport_plan[..., None] ).sum(dim=(-1,-2,-3))  -\
                              0.5 * self.edge_del_cost * corpus_adj.sum(dim=(-1,-2)) - 0.5 * self.edge_ins_cost * query_adj.sum(dim=(-1,-2))

        else:
            edge_emb_pairwise_diff = torch.maximum(nC2_query_edge_embeddings, edge_transport_plan @ nC2_corpus_edge_embeddings)
            edge_align_dist = (0.5 * (self.edge_ins_cost + self.edge_del_cost) * edge_emb_pairwise_diff ).sum(dim=(-1,-2)) -\
                               0.5 * self.edge_del_cost * corpus_adj.sum(dim=(-1,-2)) - 0.5 * self.edge_ins_cost * query_adj.sum(dim=(-1,-2))


        if self.use_h_hp_node:
            node_emb_pairwise_max = torch.maximum(stacked_query_node_features[:, :, None, :], stacked_corpus_node_features[:, None, :, :])
            node_align_dist = ((self.node_ins_cost + self.node_del_cost) * node_emb_pairwise_max * node_transport_plan[..., None]).sum(dim=(-1,-2,-3)) -\
                              self.node_del_cost * corpus_sizes - self.node_ins_cost * query_sizes

            
        else:
            node_align_dist = (self.node_ins_cost + self.node_del_cost) * torch.maximum(stacked_query_node_features, node_transport_plan @ stacked_corpus_node_features).sum(dim=(-1,-2)) -  self.node_del_cost * corpus_sizes - self.node_ins_cost * query_sizes
...
class ABL_MAX_OR(torch.nn.Module):
    def __init__(self, conf, gmn_config):
        super(ABL_MAX_OR, self).__init__()
        self.conf = conf
        self.use_max = conf.model.use_max
        self.use_second_sinkhorn = conf.model.use_second_sinkhorn
        self.use_second_sinkhorn_log = conf.model.use_second_sinkhorn_log
    ...
    def forward(self, graphs, graph_sizes, query_adj, corpus_adj):
        ...
        if self.use_m_ms_edge:
            # edge_emb_pairwise_diff : (batch_size, nC2, nC2, emb_dim)
            edge_emb_pairwise_max = torch.maximum(nC2_query_edge_embeddings[:, :, None, :], nC2_corpus_edge_embeddings[:, None, :, :])
            edge_align_dist = (0.5 * (self.edge_del_cost + self.edge_ins_cost) * edge_emb_pairwise_max * edge_transport_plan[..., None] * pairwise_or[..., None]).sum(dim=(-1,-2,-3))  -\
                              0.5 * self.edge_del_cost * corpus_adj.sum(dim=(-1,-2)) - 0.5 * self.edge_ins_cost * query_adj.sum(dim=(-1,-2))

        else:
            edge_emb_pairwise_diff = torch.maximum(nC2_query_edge_embeddings, edge_transport_plan @ nC2_corpus_edge_embeddings)
            edge_align_dist = (0.5 * (self.edge_ins_cost + self.edge_del_cost) * edge_emb_pairwise_diff * pairwise_or).sum(dim=(-1,-2)) -\
                               0.5 * self.edge_del_cost * corpus_adj.sum(dim=(-1,-2)) - 0.5 * self.edge_ins_cost * query_adj.sum(dim=(-1,-2))
