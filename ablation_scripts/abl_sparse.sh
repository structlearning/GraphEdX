...
CUDA_VISIBLE_DEVICES=$1 python -m src.train_graphedx model.name=$model dataset.name=$DATASET_NAME\
                                  training.batch_size=256 task.name="NewGS=MSS_0.01${output_mode}_${FILTERS_3}_${GMN_NPROPLAYERS}_s=${SCALE_FACTOR}_GN=0_LAM0.1_${info_str}" training.overwrite=True\
                                  gmn.variant=deep\
                                  gmn.filters_3=$FILTERS_3 gmn.GMN_NPROPLAYERS=$GMN_NPROPLAYERS model.output_mode=$output_mode model.LAMBDA=0.1\
