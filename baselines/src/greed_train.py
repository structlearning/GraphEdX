...
def test():
    test_data = data_gen(conf, "test", file_logger)
...
def evaluate(model, sampler):
    ...
    for i in range(n_batches):
        data, sizes, upper_bounds, lower_bounds = sampler.fetch_batched_data_by_id(i)
        query_data = data[0::2]
        ...
        pred.append(model(query_batch, target_batch).data)
...
def train():
    train_data = data_gen(conf, "train", file_logger)
    val_data = data_gen(conf, "val", file_logger)

    # parse graphs -> add features to networkx graphs
    ...
    counter = 0
    if not os.path.exists(f"{conf.base_dir}/{conf.training.weights_dir}"):
        os.makedirs(f"{conf.base_dir}/{conf.training.weights_dir}")
    ...
    while run < conf.training.num_epochs:
        ...
        for i in range(n_batches):
            ...
            target_batch = pyg.data.Batch.from_data_list(target_data)
            optimizer.zero_grad()
            out = model(query_batch, target_batch)
