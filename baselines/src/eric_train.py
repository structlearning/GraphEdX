...
def test():
    test_data = get_ged_comb_data(conf, "test")
...
def evaluate(model, sampler):
    ...
    for i in range(n_batches):
        ...
        pred.append(model(batch_data, batch_data_sizes)[0].data) 
...
def train():
    train_data = get_ged_comb_data(conf, "train")
    val_data = get_ged_comb_data(conf, "val")
    ...
    while run < conf.training.num_epochs:
        ...
        for i in range(n_batches):
            ...
            optimizer.zero_grad()
            out, l_cl = model(batch_data, batch_data_sizes)
            losses = model.compute_loss(lower_bounds, upper_bounds, out) + l_cl # ERIC TRADEMARK
            losses.backward()
            optimizer.step()
